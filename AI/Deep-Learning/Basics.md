...
We take random weight matrix and bias and proces data on neurons (to remember patterns) and reach to an output that flow is forward pass and the difference between the actual output required and the output we got is error and that's loss function
and to reduce loss function we do backward propgation in which we check the rate of chage of loss wrt to weights (gradiant or slope) using the chain rule


a->b->c->d => l

dl/dd -> rate of change of loss wrt to d => d.grad
dl/dc -> dl/dd * dd/dc => d.grad * dd/dc
dl/db -> dl/dc * dc/db => c.grad * dc/db